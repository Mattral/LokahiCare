...
const MultiFileUpload = ({ error, showList = false, files, type, setFieldValue, sx, onUpload, ...other }: UploadMultiFileProps) => {
  const [openPdfModal, setOpenPdfModal] = useState(false); // State to control the modal
  const [pdfUrl, setPdfUrl] = useState(''); // State to store the URL of the PDF
  ...
  const handleOpenPdfModal = (url: string) => {
    setPdfUrl(url); // Set the URL of the PDF to display
    setOpenPdfModal(true); // Open the PDF modal
  };

  const handleClosePdfModal = () => {
    setOpenPdfModal(false); // Close the PDF modal
  };
  ...
  return (
    <>
      ...
      >
        ...
        {files && files.length > 0 && (
          <FilesPreview
            ...
            onRemove={onRemove}
            type={type}
            onFileClick={handleOpenPdfModal} // Pass the file click handler to FilesPreview
          />
        )}
      </Box>
      ...
      {/* Integrate PdfModal here */}
      <PdfModal open={openPdfModal} onClose={handleClosePdfModal} pdfUrl={pdfUrl} />
    </>
  );
};

...
function ChatDrawer({ handleDrawerOpen, openChatDrawer, setUser, selectedUser }: ChatDrawerProps) {
  ...
  const handleClickRightMenu = (event: MouseEvent<HTMLButtonElement>) => setAnchorEl(event.currentTarget);
  ...
  const handleSearch = (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => setSearch(event.target.value);
    ...
    <Drawer
      sx={{
        ...
        '& .MuiDrawer-paper': {
          ...
          position: 'relative',
          border: 'none',
          borderRadius: !matchDownLG ? '12px 0 0 12px' : '0',
        },
      }}
      ...
      >
        <Box sx={{ p: 3, pb: 1 }}>
          <Stack spacing={2}>
            <Stack direction="row" spacing={0.5} alignItems="center">
              <Typography variant="h5" color="inherit">Messages</Typography>
              <Chip
                label="9"
                component="span"
        ...
        <Box sx={{ px: 3 }}>
          <List sx={{ '& .MuiListItemIcon-root': { minWidth: 32 } }}>
            <ListItemButton>
              <ListItemIcon><Logout variant="Bulk" /></ListItemIcon>
              <ListItemText primary="LogOut" />
            </ListItemButton>
            <ListItemButton>
              <ListItemIcon><Setting3 variant="Bulk" /></ListItemIcon>
              ...
              <Grid container spacing={1} alignItems="center" sx={{ flexWrap: 'nowrap' }}>
                ...
                <Grid item xs zeroMinWidth>
                  <Stack sx={{ cursor: 'pointer', textDecoration: 'none' }} component={Link} href="/apps/profiles/user/personal">
                    <Typography align="left" variant="h5" color="textPrimary">{user?.name}</Typography>
                    <Typography align="left" variant="caption" color="textSecondary">{user?.role}</Typography>
                  </Stack>
                </Grid>
                <Grid item>
                  ...
                  >
                    ...
                    <MenuItem onClick={handleRightMenuItemClick('do_not_disturb')}>
                      <IconButton size="small" sx={{ color: theme.palette.secondary[400] }}>
                        <MinusCirlce />
                      </IconButton>

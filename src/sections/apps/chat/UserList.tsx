...
function UserList({ setUser, search, selectedUser }: UserListProps) {
  ...
  useEffect(() => {
    if (!loading && Array.isArray(users)) {  // Ensure `users` is an array
      let result = users;
      if (search) {
      ...
      setData([]);  // Fallback to an empty array if `users` is not an array
    }
  }, [loading, search, users]);  // Added `users` as a dependency

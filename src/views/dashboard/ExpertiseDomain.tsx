...
const ExpertiseDomainPage = () => {
  ...
  const [newDomain, setNewDomain] = useState<{ ed_name: string; ed_type: string }>({
    ed_name: "",
    ed_type: "",
  });
...
const filteredDomains = domains.filter((domain) =>
    (domain.ed_name || "").toLowerCase().includes(searchQuery.toLowerCase()) // Ensure ed_name is a string
  );
  ...
  const handleEditSubmit = async () => {
    ...
    if (!editDomain || !editDomain.ed_name || !editDomain.ed_type) {
    ...
    formData.append("ed_name", editDomain.ed_name);  // Add ed_name field
    formData.append("ed_type", editDomain.ed_type);  // Add ed_type field
...
const handleAddSubmit = async () => {
    ...
    // Prepare FormData
    const formData = new FormData();
    formData.append("ed_name", newDomain.ed_name); // Add ed_name field
    formData.append("ed_type", newDomain.ed_type); // Add ed_type field
    ...
    <Container>
      ...
      <div className="row justify-content-center">
        <div className="col-md-10">
            ...
            <div className="mb-2" style={{ textAlign: 'center' }}>
                <Title>Advisor Expertise Domains</Title>
                <a>Domains on which an advisor can give consulting service on your app.</a>
            </div>
            ...
            {loading ? (
                ...
                <tbody>
                    ...
                    <TableRow key={domain.idDom}>
                        <TableData>{domain.ed_code}</TableData>
                        <TableData>{domain.ed_name}</TableData>
                        <TableData>{domain.ed_type}</TableData>
                        <TableActions>
      ...
      {editModalVisible && (
        <ModalBackground isVisible={editModalVisible}>
            ...
            <ModalInput
                type="text"
                value={editDomain?.ed_name || ""}
                onChange={(e) =>
                setEditDomain((prev) =>
                    prev ? { ...prev, ed_name: e.target.value } : prev
                )
                }
            ...
            <ModalSelect
              value={newDomain.ed_type}
              onChange={(e) =>
                setNewDomain({ ...newDomain, ed_type: e.target.value })
              }
            >
      ...
      {addModalVisible && (
        <ModalBackground isVisible={addModalVisible}>
          <ModalContent>
            ...
            <ModalInput
              type="text"
              value={newDomain.ed_name}
              onChange={(e) =>
                setNewDomain({ ...newDomain, ed_name: e.target.value })
              }
              placeholder="Domain Name"
            />
